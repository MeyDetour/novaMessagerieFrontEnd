...
buttonLogin.addEventListener('click', () => {
    ...
    let token = messengerLogin(nom.value,mdp.value)
})
bouttonSend.addEventListener('click', () => {
    let inputPostMessage = document.querySelector('.MsgInput')
    postMessage(inputPostMessage.value)
})

async function messengerLogin(nom,mdp){
    const utilisateur = {
        username: nom,
        password: mdp,
    }
    ...
    //login
    fetch(`${baseUrl}login`, messengerLogin)
...
async function recupererMessage() {
    ...
    await fetch(`${baseUrl}api/messages`, messengerMessage)
        .then(response => response.json())
        .then(data => {

            document.querySelector('.filDiscussion').classList.toggle('d-none')
...
async function afficheMessage(listemessage) {
    listemessage.forEach((message) => {
        console.log(message)
        document.querySelector('.messages').innerHTML += `                    
                              <div class="task" draggable="true">
                                <div class="tags">
                                  <span class="tag">${message['author']['username']}</span>
...
async function postMessage(message) {
    ...
    fetch(`${baseUrl}messages/new`, messengerMessage)
        .then(response => response.json())
        .then(data => {
            console.log(data)
            recupererMessage(token)
